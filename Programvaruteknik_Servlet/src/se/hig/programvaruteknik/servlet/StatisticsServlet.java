...
public class StatisticsServlet extends HttpServlet
{
    ...
    private BiFunction<Boolean, DataCollection, String> JSONFormatter;
	...
	this((pretty, source) ->
	{
	    return new DataCollectionToJSONConverter(source, pretty).getString();
	});
    }
     ...
     */
    public StatisticsServlet(BiFunction<Boolean, DataCollection, String> JSONFormatter)
    {
    ...
    private <E extends Enum> E getEnum(Class<E> enumType, String name, String onError)
    {
	try
	{
    ...
    private String getPart(String[] all, Integer index, String onError)
    {
	if (index >= all.length) throw new RequestException(onError);
	return all[index];
    }

    private DataSourceBuilder getDataSource(String parameters, String name)
    {
    ...
    private boolean getPretty(HttpServletRequest request)
    {
	...
	try
	{
	    ...
	    DataCollectionBuilder builder = new DataCollectionBuilder(sourceA.build(), sourceB.build(), resolution);
	    DataCollectionToJSONConverter converter = new DataCollectionToJSONConverter(builder.getResult(), pretty);

	    response.setContentType("application/json;charset=UTF-8");
	    response.getWriter().append(converter.getString());
	}
